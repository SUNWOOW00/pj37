...
form.addEventListener('submit', (e) => {
    ...
    const generatedText = loremIpsum(size); // 생성된 텍스트를 가져옴
    document.querySelector('.text-section').innerHTML = generatedText; // 생성된 텍스트를 text-section에 추가
    
    // 텍스트 컨테이너를 보여주거나 숨김
    const textWrapper = document.querySelector('.text-wrapper');
    if (generatedText) {
        textWrapper.style.display = 'flex'; // 텍스트가 있으면 보여줌
    } else {
        textWrapper.style.display = 'none'; // 텍스트가 없으면 숨김
    }
});
...
function loremIpsum(size) {
    ...
    Nunc purus purus bibendum urna, at scelerisque quam libero sed arcu. Fusce eget purus eget purus sodales tristique ac nec nunc. Pellentesque eleifend laoreet
    viverra sit amet justo in lacinia. Vestibulum scelerisque, orci id cursus placerat,
    ultrices.`;
...
copy.addEventListener('click', (e) => {
    ...
    document.body.removeChild(textArea); // 임시 텍스트 영역 제거
});
